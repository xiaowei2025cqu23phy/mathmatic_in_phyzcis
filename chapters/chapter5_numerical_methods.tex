...
前四章建立了物理问题的解析框架：分离变量、特殊函数、积分变换。但现实世界远比教科书复杂——非均匀介质、非线性项、不规则边界，这些都会使解析解遥不可及。
...
本章以\textbf{有限差分法} 为主线，结合 Python 的 NumPy/SciPy 生态，让你亲手"计算"前四章的偏微分方程。你将看到分离变量法给出的本征函数如何转化为矩阵本征值问题，傅里叶变换如何用 FFT 在 $O(N\log N)$ 时间内完成。
...
\begin{itemize}
\item \textbf{物理专业学生：} 重点关注 \S5.2--\S5.5（有限差分基础与 PDE 离散化），\S5.7（矩阵本征值问题——量子力学的数值视角），\S5.9（FFT 与谱方法）。
\item \textbf{计算科学学生：} 重点关注 \S5.2（截断误差与收敛性分析），\S5.5--\S5.6（不同时间步进格式的稳定性），\S5.8（误差分析与网格收敛性研究）。
\item \textbf{工程学生：} 重点关注 \S5.3--\S5.6 中可运行的数值算例代码，以及 \S5.11 的综合案例，直接应用于实际问题。
\end{itemize}
...
# 边界条件: 上边界 y=1 处 u=sin(pi*x), 其余边界 u=0
    f = np.zeros(N)
...
\subsection{迭代法初步：Jacobi、Gauss-Seidel 与 SOR}

直接法（Thomas 算法、\texttt{spsolve}）适合中小规模问题；当网格点数达到 $10^5$ 量级时，\textbf{迭代法}——每步只做一次矩阵-矢量乘法，内存占用 $O(N)$——成为更现实的选择。

以二维拉普拉斯方程的五点格式 $4u_{ij} = u_{i+1,j}+u_{i-1,j}+u_{i,j+1}+u_{i,j-1}$ 为例，设第 $k$ 次迭代的近似值为 $u^{(k)}_{ij}$：

\textbf{Jacobi 迭代}（同步更新）：
\begin{equation}
u^{(k+1)}_{ij} = \frac14\left(u^{(k)}_{i+1,j}+u^{(k)}_{i-1,j}+u^{(k)}_{i,j+1}+u^{(k)}_{i,j-1}\right).
\label{eq:jacobi}
\end{equation}

\textbf{Gauss-Seidel 迭代}（按行扫描，立即使用刚算出的新值）：
\begin{equation}
u^{(k+1)}_{ij} = \frac14\left(u^{(k)}_{i+1,j}+u^{(k+1)}_{i-1,j}+u^{(k)}_{i,j+1}+u^{(k+1)}_{i,j-1}\right).
\label{eq:gauss_seidel}
\end{equation}

\textbf{SOR（逐次超松弛）}：在 Gauss-Seidel 基础上外推，松弛因子 $\omega\in(0,2)$：
\begin{equation}
u^{(k+1)}_{ij} = (1-\omega)u^{(k)}_{ij} + \omega\,\hat{u}^{(k+1)}_{ij},
\label{eq:sor}
\end{equation}
其中 $\hat{u}^{(k+1)}_{ij}$ 是 Gauss-Seidel 的更新值。$\omega<1$ 为欠松弛，$\omega>1$ 为超松弛；对单位正方形上的五点格式，最优因子为 $\omega_{\mathrm{opt}} \approx 2/(1+\sin\pi h)$，收敛速度比 Jacobi 快一个数量级。

\textbf{收敛性：} 对角占优或正定矩阵保证 Jacobi 与 Gauss-Seidel 收敛，收敛速度由迭代矩阵的谱半径 $\rho$ 决定。对 $M\times M$ 网格的五点格式，Jacobi 的谱半径为 $\rho(J)=\cos(\pi h)$（$h=1/(M+1)$），达到给定精度所需的迭代次数按 $O(M^2)$ 增长；Gauss-Seidel 的谱半径约为 $\rho(J)^2$（收敛快一倍）；SOR 取最优因子时谱半径 $\sim 1-O(h)$，收敛再快一个量级。

\textbf{停机准则：} 常用相邻两次迭代的最大改变量 $\|u^{(k+1)}-u^{(k)}\|_\infty < \varepsilon$，或残差范数 $\|Au^{(k)}-f\| < \varepsilon$。
...
如果 $r>1/2$，数值解会指数增长。原因在于显式格式的放大因子
\[
|g(k)| = \left|1 - 4r\sin^2\frac{kh}{2}\right| > 1
\]
对高频模成立。

\begin{insight}[CFL 条件的数值解读]
CFL 条件 $r \leq 1/2$ 的数值解读：它等价于要求放大因子 $|g(k)| \leq 1$ 对所有波数 $k$ 成立（von Neumann 稳定性分析）。注意热传导方程是抛物型的，物理上信息的传播速度是无穷的（解在 $t>0$ 后立即处处非零），因此 $r\leq1/2$ 纯粹来自数值稳定性要求，与 \S5.6 波动方程中"数值依赖域必须包含物理依赖域"的因果性条件（$c\Delta t/h \leq 1$）性质不同，不可混为一谈。
\end{insight}
...
def wave_1d(Nx=200, c=1.0, T_final=2.0):
    ...
    return x, u_curr
# 初始静止高斯波包分裂为两个半波包分别向 +x 与 -x 传播
# 计算域为 [-1,1]（边界点恒为 0，等价固定端），t 约 1 时波包已触边并反射
\end{lstlisting}
...
# 1   1.233696   1.233701   4.1e-06
# 2   4.934737   4.934802   6.5e-05
\end{lstlisting}
...
# 基态误差约 3e-6；误差随 n 增大而增大（n=2 约 4e-5）
\end{lstlisting}
...
# 验证 Parseval 定理
E_time = np.trapezoid(f**2, t)
E_freq = np.trapezoid(np.abs(F)**2, freq) / fs**2
...
\begin{equation}
\tilde{f}_n = \sum_{k=0}^{N/2-1} f_{2k} e^{-2\pi\mathrm{i} nk/(N/2)}
+ e^{-2\pi\mathrm{i} n/N}\sum_{k=0}^{N/2-1} f_{2k+1} e^{-2\pi\mathrm{i} nk/(N/2)}.
\end{equation}
...
\begin{insight}
对于一维泊松方程，线性有限元与中心差分导出的矩阵完全相同（严格地说，FEM 刚度矩阵与 FD 矩阵相差因子 $h$，右端项亦同，故解完全相同）！但 FEM 在处理\textbf{非均匀网格}和\textbf{复杂边界}时更加灵活——自然边界条件（Neumann）在弱形式中自动满足，不需要特殊处理。
\end{insight}
...
def steady_heat_source(N=200):
    ...
    # 右边界 Neumann: 末行离散方程为 (u_N-u_{N-1})/h^2 = f_N，
    # 因 f(1) 约等于 0，数值上相当于 u'(1) 约等于 0（一阶 Neumann 近似）
    A[N, N-1] = -1 / h**2
    A[N, N] = 1 / h**2
...
\textbf{连续 PDE $\to$ 网格离散 $\to$ 差分近似 $\to$ 稀疏矩阵 $\to$ 代数方程组求解。} 这个链条将前四章的所有解析工具（微分算符、傅里叶变换、特殊函数）统一为可计算的数值框架。你可以将本章看作前四章的"数值镜像"——每当你学到一个新的解析方法，都应该想一想："这个问题的离散版本是什么？对应的矩阵长什么样？"
\end{insight}
...
\begin{enumerate}
\item 修改 \texttt{poisson\_1d} 函数，将右端项改为 $f(x)=x(1-x)$，并与解析解 $u(x)=\frac{1}{12}x(1-x)(1+x-x^2)$ 比较误差。验证误差随 $h$ 以 $O(h^2)$ 衰减。
...
\item[11.] 对二维拉普拉斯方程使用 \S5.4 介绍的迭代求解方法（Jacobi 迭代、Gauss-Seidel 迭代、SOR 方法），比较收敛速度。\\
\textbf{提示：} 取 $M=20$ 的网格，画出误差随迭代步数的变化曲线。
...
\item[14.]（薛定谔方程的时间演化）用 Crank-Nicolson 格式离散化时间相关薛定谔方程 $\mathrm{i}\hbar\partial_t\psi = -\frac{\hbar^2}{2m}\partial_x^2\psi$，编写一个演化高斯波包的代码。\textbf{提示：} 实时间薛定谔方程是色散方程（波包振荡并扩散），并非二阶波动方程；令 $\tau=\mathrm{i}t$（虚时间）才化为热传导型方程。用 Crank-Nicolson 离散时，$\mathrm{i}$ 使格式保持幺正而非耗散。
