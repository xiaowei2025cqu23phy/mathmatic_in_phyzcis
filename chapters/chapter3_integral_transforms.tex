...
\textbf{时域微分性质} 尤为重要：它将微分方程化为代数方程。这正是傅里叶变换求解微分方程的关键。卷积定理则是线性系统分析的核心——时域中的卷积对应于频域中的乘法，这意味着系统的输出谱等于输入谱乘以传递函数。
...
\mathcal{F}[e^{-at^2}] &= \sqrt{\frac{\pi}{a}}\,e^{-\omega^2/(4a)},\quad a>0,\\
...
\textbf{高斯函数的傅里叶变换仍是高斯函数}——这一性质在量子力学中解释为什么高斯波包是最小不确定波包。更精确地说，$f(t)=e^{-t^2/2}$ 的变换为 $\sqrt{2\pi}\,e^{-\omega^2/2}$，即它（在相差一个常数因子的意义下）是傅里叶变换的本征函数。
...
这里采用量子力学惯用的对称归一化约定，与本节其余部分 $\tilde f(\omega)=\int f(t)e^{-\mathrm{i}\omega t}\dd t$（逆变换带 $1/2\pi$）的约定只差一个归一化因子，两者在物理上是等价的。
...
&\leq 4\int |t f(t)|^2\,\dd t \cdot \int |f'(t)|^2\,\dd t \\
&= 4\,\Delta t^2 \cdot (\Delta\omega)^2 \cdot \left(\int |f|^2\,\dd t\right)^2,
\end{align}
（第二步用到了帕塞瓦尔定理 $\int|f'|^2\,\dd t = \frac{1}{2\pi}\int\omega^2|\tilde f(\omega)|^2\,\dd\omega$），由此得 $\Delta t\cdot\Delta\omega \geq 1/2$。
...
\begin{equation}
\mathcal{L}[f(t)] = F(s)
= \int_{0^-}^{\infty} f(t)\,e^{-st}\,\dd t,\qquad s = \sigma + \mathrm{i}\omega.
...
\end{equation}
其中 $A_{1,k} = \frac{1}{(m-k)!}\lim_{s\to p_1}\frac{\dd^{m-k}}{\dd s^{m-k}}\left[(s-p_1)^m\frac{N(s)}{D(s)}\right]$（$k=1,\dots,m$）。
...
因此：
\begin{equation}
Y(s) = \frac{s}{(s+1)^2 + 2^2} + \frac{2-s}{s^2+1}.
\end{equation}

\textbf{步骤 4：逆变换（查表）。}
\begin{align*}
y(t) &= e^{-t}\cos 2t - \frac12 e^{-t}\sin 2t - \cos t + 2\sin t.
\end{align*}
...
\item[11.] 一个二阶低通滤波器 $H(s) = \frac{\omega_0^2}{s^2 + 2\alpha s + \omega_0^2}$。讨论 $\alpha=0$（无阻尼）、$\alpha<\omega_0$（欠阻尼）、$\alpha=\omega_0$（临界阻尼）时的单位阶跃响应，并画出阶跃响应 $s(t)$ 的波形。
\item[12.] 求 $f(t) = \cos(100\pi t)$ 的傅里叶变换（信号频率为 $50$ Hz）。如果以 $f_s=80$ Hz 采样，为什么会出现混叠（aliasing）现象？混叠后观察到的频率是多少？
\end{enumerate}
...
\item[18.]（分数阶傅里叶变换）分数阶傅里叶变换 $F^{\alpha}[f]$ 是传统 FT 的推广。说明当 $\alpha=0$ 时就是恒等变换，$\alpha=\pi/2$ 时是标准傅里叶变换，$\alpha=\pi$ 时是时间反转。讨论其在光学传播（Fresnel 衍射）与量子力学波包演化中的应用。
\end{enumerate}
